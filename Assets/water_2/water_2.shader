Shader "Custom/Ripple" {
  ...
  SubShader {
    ...
    void surf (Input IN, inout SurfaceOutputStandard o) {
      ...
      //赋值到法线上      
      float dis = length(IN.uv_MainTex - 0.5) * 2 * 3;
      if(dis > 1)
      dis = 0;
      float dropFrac = 1 - saturate(frac(_Time.y));
      float final = dropFrac * sin(clamp((dropFrac - 1.0 + dis) * 9, 0.0, 6.0) * PI);
      o.Normal = float3(dis * final, dis * final , 1);
      o.Alpha = c.a;
    }
    ENDCG
  }
